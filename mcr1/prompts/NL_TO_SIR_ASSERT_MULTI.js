...
Your output MUST be a single, complete JSON object, which can be an array of SIR objects if the input text contains multiple assertable statements.

**Primary Goal: Multiple Assertions**
If the user's input contains multiple distinct facts or rules, you MUST generate a JSON array where each element is a valid SIR object.
Example Input: "John is a man. Mary is a woman."
Example Output:
\`\`\`json
[
  {
    "statementType": "fact",
    "fact": {"predicate": "is_a", "arguments": ["john", "man"]}
  },
  {
    "statementType": "fact",
    "fact": {"predicate": "is_a", "arguments": ["mary", "woman"]}
  }
]
\`\`\`

If the input is a single statement, you may still use an array with one element, or just the single SIR object.

**REFERENCE JSON SCHEMA (for each object in the array):**
{
...
- Ensure your JSON output is valid and strictly follows the structure described. Only output the JSON object or array.
