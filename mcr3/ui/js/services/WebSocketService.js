	...
	connect() {
		return new Promise((resolve, reject) => {
			...
			this.socket.onmessage = event => {
				...
				// Handle both broadcast messages and direct replies
				...
				if (message.messageId && this.listeners.has(message.messageId)) {
					...
					this.listeners.delete(message.messageId); // Clean up one-time listener
				}
			};
...
// Assumes the WebSocket server is at /ws on the same host
